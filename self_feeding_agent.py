...
from .test import test, extract, cd
...
class SelfFeedingAgent(TransformerRankerAgent):
    ...
    def act(self):
        if self.opt['request_feedback'] or self.opt['request_rating']:
            ...
            if self.status == NORMAL:
                #CODE CHANGES
                #print (self.observation['text'])
                txtExtract = self.observation['text']
                extract (txtExtract)
    ...
    def make_feedback_request(self):
    	test()
    	#return last_messag
        #return 'Oops! I think I ferrari lambo up. ' 'What could I have said instead?'
        #return (last_messag)
